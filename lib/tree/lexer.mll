...
and props tagname acc = parse
  ...
  *)
  | (word as name) "={$" (['0'-'9']+ as label) "}" { props tagname ((name, AttrFunc (Label (int_of_string label, None))) :: acc) lexbuf }
  | (word as name) "={$" (['0'-'9']+ as label) "$" (['0'-'9']+ as key) "}" { props tagname ((name, AttrFunc (Label (int_of_string label, Some (int_of_string key)))) :: acc) lexbuf }
